...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if (num_rolls == 0):
        ...
        dig2 = opponent_score % 10
        max_dig = max(dig1, dig2) + 1
        return max_dig
    else:
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while (score < goal and opponent_score < goal):
        new_dice = select_dice(score, opponent_score)
        if (score == 2 * opponent_score) or (opponent_score == 2 * score):
            score, opponent_score = opponent_score, score
        if (who == 0):
            num_rolls = strategy0(score, opponent_score)
            score = score + take_turn(num_rolls, opponent_score, new_dice)
        if (who == 1):
            num_rolls = strategy1(opponent_score, score)
            opponent_score = opponent_score + take_turn(num_rolls, score, new_dice)
        who = other(who)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for current_rolls in range(1,11):
        ...
        score = averaged_roll_dice(current_rolls, dice)

        print(str(current_rolls) + ' dice scores ' + str(score) + ' on average')
...
def bacon(opponent_score):

    def single_nums():    
        return list(map(int, str(opponent_score)))

    return max(single_nums()) + 1
...
def bacon_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    if bacon(opponent_score) >= BACON_MARGIN:
        return 0
...
def swap_strategy(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    def new():
        return score + bacon(opponent_score) 

    if (opponent_score == new() * 2):
        return 0

    elif (opponent_score == new()/2):
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    Building off of the bacon strategy. Maximizes their four sided rolls"""
    "*** YOUR CODE HERE ***"
    my_score = opponent_score // 10
    opp_score = opponent_score % 10
    max_num = max(my_score, opp_score) + 1
    new_score = max_num + score
    if (opponent_score == 0): 
        return BASELINE_NUM_ROLLS
    elif (select_dice(new_score, opponent_score) == four_sided):
        return 0
    elif (opponent_score // new_score == 2):
        return 0
